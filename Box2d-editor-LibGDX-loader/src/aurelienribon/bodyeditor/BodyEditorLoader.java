...
public class BodyEditorLoader {
	 ...
	 * @param centerMass 
	 * 			  If the default center should be used.
	 */
	public void attachFixture(Body body, String name, FixtureDef fd,
			float scale, boolean centerMass) {
		...
		Vector2 origin = null;
		if (centerMass) {
			origin = vec.set(rbModel.origin).mul(scale);
		} else {
			origin = newVec();
		}
	 ...
	 * @return
	 */
	public ArrayList<PolygonShape> getShapes(String name, float scale) {
		ArrayList<PolygonShape> arr = new ArrayList<>();
		RigidBodyModel rbModel = model.rigidBodies.get(name);
		if (rbModel == null)
			throw new RuntimeException("Name '" + name + "' was not found.");
	...
	private Model readJson(String str) {
		...
		for (int i = 0; i < bodiesElems.size; i++) {
			RigidBodyModel rbModel = readRigidBody(bodiesElems.get(i));
